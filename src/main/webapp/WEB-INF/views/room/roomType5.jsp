...
<body>
	<div>
		<form class="form-horizontal col-md-8" action="checkRoomOrders">
			<table class="table" id="user">
				...
				<tbody>
					<tr id="userAdd">
						<td class="col-xs-2"><select id="roomNum"
						name="roomNum" class="selectpicker form-control"
						data-live-search="true">
						<option value="">全部</option>
						<c:forEach items="${rooms}" var="list">
						<c:if test="${list.roomNum == param.roomNum}">
							<option value="${list.roomNum}"  selected = "selected">${list.roomNum}${list.roomSize.sizeName}</option>
						</c:if>
						<c:if test="${list.roomNum != param.roomNum}">
						...
						<td class="col-xs-3"><input type="text" class="form-control"
							name="userId" value="${param.userId}" placeholder="请输入搜索身份证号码"></td>
							<td class="col-xs-1"><input type="text" value="${param.pageSize}"  class="form-control" name="pageSize"></td>
						<td class="col-xs-3"><input name="startTime"
							class="form-control" value="${param.startTime}" id="startTime" />~
							<input name="endTime" class="form-control"
							value="${param.endTime}" id="endTime" /></td>
		...
		<div>
			<div class="col-md-10 column">
							...
							<tbody>
								...
								<c:forEach items="${pagehelper.list}" var="list" varStatus="i">
									<tr>
										...
										<td>${list.roomOrdersEndTime.toString().substring(0,(list.roomOrdersStartTime.toString().length()-2))}</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
				...
				<c:if test="${pagehelper.list.isEmpty() }">
					<h3>
						<span class="label label-danger">暂无房间订单数据</span>
					</h3>
				</c:if>
			</div>
		</div>
	...
	<script>
		...
		function toExcel(tableid) {
			...
			var fname = oXL.Application.GetSaveAsFilename("房间订单.xlsx",
					"Excel工作簿(*.xlsx), *.xlsx");
			oWB.SaveAs(fname);
			oWB.Close();
			oXL.Quit();
